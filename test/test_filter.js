...
describe('Convert query strings filter attribute into Sequelize find queries.', (done) => {
	...
	it('(eq) throw success if prop in allowed prop list', () => {
		let qs = 'hello eq 4301';
		try {
			filter.find(qs, ['hello', 'world']);
		} catch (e) {
			expect(e).to.be.instanceof(Error);
			expect(e.message).to.be.equal('The property "geoId" is not allowed');
		}
	});
	it('(eq) throw if a property not in allowed prop list', () => {
